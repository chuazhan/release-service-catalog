...
function oras() {
  ...
  # Mock oras pull - create dummy RPM files
  if [[ "$*" == "pull "* ]]; then
    echo "Creating mock RPM files in current directory..."  >> /dev/stderr
    touch test-rpm-1.0-1.el9.x86_64.rpm
    touch test-rpm-1.0-1.el9.src.rpm
    echo "Created 2 mock RPM files"  >> /dev/stderr
  # Mock manifest fetch for RPM components
  elif [[ "$*" == "manifest fetch "* ]]; then
    echo '{
      "layers": [
        {
          "annotations": {
            "org.opencontainers.image.title": "test-rpm-1.0-1.el9.x86_64.rpm"
          }
        },
        {
          "annotations": {
            "org.opencontainers.image.title": "test-rpm-1.0-1.el9.src.rpm"
          }
        }
      ]
    }'
  # Mock oras push - just echo
  elif [[ "$*" == "push "* ]]; then
    echo "Mock oras push called"  >> /dev/stderr
  else
